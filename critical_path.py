...
def CallbackItemOfActivity(index: int, row, activity_manager: ActivityManager): 
    values = {}
...
def ConvertDataToEntity(data, callbackItem: None, activity_manager: ActivityManager): 
    res = []
     
    for index, row in data.iterrows():
        item: Activity = callbackItem(index, row, activity_manager)
        res.append(item)
...
"""
    ...
    - 
"""
def traceCriticalPath(activity_manager: ActivityManager):
...
def calculateCriticicalPath(activity_manager: ActivityManager): 
    d_activities = deque()
    ...
    while(len(d_activities) != 0):
        ...
        for activity_number in next_activities:
            ...
            activity_item.values['EF'] = int(activity_item.values['ES']) + int(activity_item.values['duration'])
    ...
    for activity_number  in end_activity_numbers:
        ...
        activity_item.values['LS'] = int(activity_item.values['LF']) - int(activity_item.values['duration'])
        d_activities.append(activity_item)
...
def find_path(df: pd.DataFrame):
    
    
    activity_manager = ActivityManager()
    ConvertDataToEntity(df, callbackItem= CallbackItemOfActivity, activity_manager = activity_manager)
    activity_manager.run()


    calculateCriticicalPath(activity_manager)
    paths = traceCriticalPath(activity_manager=activity_manager)
...
def convertCriticalPathToDataFrameForMinZ(activity_file, df_for_critical_path) -> pd.DataFrame:
    activity_file.columns = ['Activity']
    res_file = activity_file.merge(df_for_critical_path, how = 'inner', on = ['Activity'])


    fields = ['Cost_of_min_model', 'Cost_of_normal_model', 'Duration_of_normal_model','Duration_of_min_model']

    for field in fields:
        res_file[field] = res_file[field].astype(int)
    
    res_file['C_ij'] = abs(res_file['Cost_of_min_model'] - res_file['Cost_of_normal_model']) / abs(res_file['Duration_of_min_model'] - res_file['Duration_of_normal_model'])
    res_file['C_ij'] = res_file['C_ij'].fillna(0)
